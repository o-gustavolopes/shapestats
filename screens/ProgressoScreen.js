...
export default function ProgressoScreen() {
    ...
    <View style={styles.card}>
      ...
      {datasets[0].data.length >= 2 ? (
        <LineChart
          ...
          withDots withShadow withInnerLines withOuterLines
          yAxisSuffix={` ${suffix}`}
          xLabelsOffset={-4}
  ...
  if (metaPeso != null && pesoValues.length >= 2) {
    datasetsPeso.push({ data: new Array(pesoValues.length).fill(metaPeso), withDots: false, color: (o=1)=>`rgba(231,76,60,${o})`, strokeWidth: 2 });
  }

  const medidasChrono = toChrono(medidas, 12);
  const build = (key) => ({ labels: medidasChrono.map(r=>r.data), data: medidasChrono.map(r=>Number(r[key])||0) });
  const c = build("cintura"), p = build("peito"), b = build("braco"), x = build("coxa");
  ...
  return (
    <ScrollView contentContainerStyle={{ paddingVertical: 20, paddingHorizontal: 16 }}>
      ...
      <Section title={`Peso${metaPeso!=null?` · Meta: ${metaPeso.toFixed(1)} kg`:''}`} labels={pesoLabels} datasets={datasetsPeso} suffix="kg" />
